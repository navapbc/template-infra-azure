...
// Prerequisite: Ensure the container image for the current git hash has  been
// built and published to the container image repository (or set the IMAGE_TAG
// env var to a tag that has).
//
...
func TestService(t *testing.T) {
	...
	TerraformInit(t, terraformOptions, "dev.azurerm.tfbackend")
	...
	WaitForServiceToBeStable(t, workspaceName, terraformOptions)
	RunEndToEndTests(t, terraformOptions)
}

func WaitForServiceToBeStable(t *testing.T, workspaceName string, terraformOptions *terraform.Options) {
	fmt.Println("::group::Wait for service to be stable")

	serviceEndpoint := terraform.Output(t, terraformOptions, "service_endpoint")

	fmt.Printf("Pinging service to start up instance: %s\n", serviceEndpoint)
	http.Get(serviceEndpoint + "/health")
...
func getImageTag(t *testing.T) string {
	if imageTag, ok := os.LookupEnv("IMAGE_TAG"); ok {
		return imageTag
	}
